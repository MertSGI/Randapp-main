-- =========================================================================
-- MIGRATION 20260831_p1c_public_branch_read_contract_runtime_fix.sql
-- Description: Forward-only runtime fix for public.get_public_branches(p_slug text)
-- Aligns public branch discovery eligibility with canonical can_accept_public_booking contract,
-- requiring status IN ('active', 'manual_active'), onboarding_status = 'completed', and public_site_status = 'published'.
-- =========================================================================
...
BEGIN
  ...
  INTO v_tenant_id, v_status, v_onboarding_status, v_public_status
  FROM public.tenants
  ...
  IF v_status NOT IN ('active', 'manual_active') 
     OR v_onboarding_status IS DISTINCT FROM 'completed'
     OR v_public_status IS DISTINCT FROM 'published' THEN
    RETURN jsonb_build_object(
      'success', false,
