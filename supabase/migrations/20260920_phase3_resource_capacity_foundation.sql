...
-- =========================================================================
-- 8. Atomic Booking Primitive with Resource Allocation
-- =========================================================================
-- Integrates slot locking, appointment insertion, and resource allocation in ONE transaction.
-- If resource allocation fails or rolls back, appointment creation rolls back.
...
DECLARE
    ...
    v_qty               INTEGER;
BEGIN
    -- 1. Canonical staff advisory lock
    v_lock_key := hashtextextended(
        p_tenant_id::text || ':' || p_staff_id::text || ':' || p_appointment_date::text,
        0
    );
    PERFORM pg_advisory_xact_lock(v_lock_key);

    -- 2. Validate Service Duration
    SELECT duration INTO v_svc_duration
    FROM public.services
    ...
    -- 3. Evaluate and lock complete resource allocation plan in stable order
    v_res_eval := public.evaluate_and_lock_resource_plan(
        p_tenant_id,
        p_branch_id,
        p_service_id,
        ...
        v_svc_duration,
        NULL,
        TRUE -- acquire transaction advisory locks
    );
    ...
    INSERT INTO public.appointments (
        ...
        p_tenant_id, p_branch_id, p_customer_id, trim(p_customer_name),
    ...
    -- 5. Allocate all resources in the plan atomically
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_res_eval->'allocation_plan')
    LOOP
        v_res_id := (v_item->>'resource_id')::UUID;
        v_qty    := COALESCE((v_item->>'quantity')::INTEGER, 1);

        INSERT INTO public.appointment_resources (tenant_id, appointment_id, resource_id, quantity)
        VALUES (p_tenant_id, v_appointment_id, v_res_id, v_qty);
    END LOOP;
