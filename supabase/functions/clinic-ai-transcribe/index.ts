...
serve(async (req: Request) => {
  ...
  try {
    ...
    let successfulResult: ClinicTranscriptionProviderResult | null = null;
    ...
    for (const candidate of candidates) {
      ...
      try {
        successfulResult = await providerInstance.transcribe({
          audio: audioBytes,
        ...
        if (err instanceof ClinicAiProviderApiError && err.isRetryable) {
          console.warn(`[clinic-ai-transcribe] Retryable provider error from ${err.providerName} (status=${err.statusCode || 'network'}). Proceeding to fallback if available.`);
    ...
    if (successfulResult) {
      return new Response(
        JSON.stringify({
          success: true,
          data: {
            transcript: successfulResult.transcript,
            detectedLanguage: successfulResult.detectedLanguage || null,
            providerRequestId: successfulResult.providerRequestId || null,
          },
        }),
