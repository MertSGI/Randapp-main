...
assert(migrationSql.includes('REFERENCES public.customers(id, tenant_id)'), 'Reuses canonical public.customers with composite tenant integrity');
...
assert(migrationSql.includes("('tenant_owner', 'staff')"), 'Uses canonical application roles tenant_owner and staff');
assert(!migrationSql.includes("role IN ('admin', 'staff', 'owner')"), 'Eliminates non-canonical role references (admin/owner)');
assert(migrationSql.includes('SET search_path = pg_catalog, public'), 'Hardens search_path to pg_catalog, public');
assert(migrationSql.includes('FOREIGN KEY (segment_id, tenant_id)'), 'Enforces composite tenant FK on segment_id');
assert(migrationSql.includes('FOREIGN KEY (customer_id, tenant_id)'), 'Enforces composite tenant FK on customer_id');
assert(migrationSql.includes('DYNAMIC_RULE_EVALUATION_NOT_IMPLEMENTED'), 'Explicitly classifies dynamic rule evaluation as NOT_IMPLEMENTED');
assert(migrationSql.includes('is_staff_assigned_to_branch'), 'Provides branch-sensitive permission checking helper');
