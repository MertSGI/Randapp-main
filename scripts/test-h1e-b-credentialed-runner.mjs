...
export async function runCredentialedMutationAcceptanceH1EB() {
  ...
  try {
    ...
    for (const roleKey of roles) {
      ...
      if (!authRes.ok || !authRes.token) {
        loginFailed = true;
        console.error(`  ❌ SETUP FAIL: Login failed for role ${cred.label}`);
        break;
      }
    ...
    if (loginFailed) {
      console.error('Aborting runner execution due to identity login failure.');
      process.exit(1);
    }
    ...
    const anonAclOk = assertAnonAclDenied(anonReadRes);
    trackResult('auth', anonAclOk, 'Anon call denied at RPC ACL boundary');
    ...
    for (const uRole of unauthorizedRoles) {
      ...
      const uOk = assertAuthenticatedUnauthorized(uRes);
      trackResult('auth', uOk, `Role ${uRole} receives UNAUTHORIZED on read RPC`);
      ...
      const uApproveOk = assertAuthenticatedUnauthorized(uApproveRes);
      trackResult('auth', uApproveOk, `Role ${uRole} receives UNAUTHORIZED on approve RPC`);
    }
  ...
  forbiddenRequestsDetected = observer.getForbiddenAttempts();
  forbiddenMutationAttempts = observer.getForbiddenAttempts();
