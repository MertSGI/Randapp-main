...
function testPublicBookingTests27_28_47HarnessContracts() {
  console.log('--- Testing Tests 27-28 & 47 Hardened Slot Object Harness Contracts (R9-R1.8.11.1) ---');
  const sqlPath = path.join(__dirname, '..', 'supabase/tests/public_booking_rpc_behavioral_tests.sql');
  const content = fs.readFileSync(sqlPath, 'utf8');
  ...
  // 3. EXACT TEST 47 REGION ISOLATION & ORDERING
  ...
  const test47RpcInRegion = test47Region.indexOf('public.get_public_available_slots');
  const test47ReasonInRegion = test47Region.indexOf('TEST 47 FAIL: second slot query failed');
  const test47TypeofInRegion = test47Region.indexOf("jsonb_typeof(v_slots) IS DISTINCT FROM 'array'");
  const test47ElementsInRegion = test47Region.indexOf('jsonb_array_elements(v_slots)');
  const test47StartFieldInRegion = test47Region.indexOf("elem.slot->>'start' = v_first_slot");
  const test47PassInRegion = test47Region.indexOf('TEST 47 PASS');

  if (
    test47RpcInRegion === -1 ||
    test47ReasonInRegion === -1 ||
    test47TypeofInRegion === -1 ||
    test47ElementsInRegion === -1 ||
    test47StartFieldInRegion === -1 ||
    test47PassInRegion === -1
  ) {
  ...
  if (
    !(
      test47RpcInRegion < test47ReasonInRegion &&
      test47ReasonInRegion < test47TypeofInRegion &&
      test47TypeofInRegion < test47ElementsInRegion &&
      test47ElementsInRegion < test47StartFieldInRegion &&
      test47StartFieldInRegion < test47PassInRegion
    )
  ) {
...
function main() {
  ...
  console.log('\n🎉 ALL HARDENED R9-R1.8.11.1 CONTRACT SELF-TESTS PASSED!');
}
