...
We use React Router's `BrowserRouter` instead of `HashRouter`. This allows for clean, "real" URLs (like `LAR\u0130.com/admin` instead of `LAR\u0130.com/#/admin`).
...
1. **Subdomains:** If `VITE_APP_BASE_DOMAIN` is set to `LAR\u0130.com`, and a user accesses `demo.randevulari.com`, the service extracts `demo` and performs a database query looking for `slug = 'demo'`.
2. **Custom Domains:** If the user accesses `salon.com` and it doesn't end with `LAR\u0130.com`, the application natively searches for `custom_domain = 'salon.com'`.
