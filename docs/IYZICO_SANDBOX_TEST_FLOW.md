...
2. **Access Payment Test Diagnostics**: Log in as `superadmin@randevulari.com` and navigate to `/#/super-admin/payment-test`.
3. **Verify Configuration**: Click "Run Health Check" and ensure all variables return true (without exposing values).
4. **Test Checkout Session Generation**: Select a plan and click "Test Checkout Session OluÅŸtur". Inspect the JSON response block for `ok: true`.
...
4. **Click "Bu plana geÃ§":** Initiate the upgrade process.
