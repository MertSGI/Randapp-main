...
- [ ] Log in as `admin@randevulari.com`. Verify routing goes directly to `/admin`.
- [ ] As `admin@randevulari.com`, manually try to navigate to `/super-admin`. Verify "Bu alana eriÅŸim yetkiniz yok" view appears.
- [ ] Log in as `superadmin@randevulari.com`. Verify routing goes directly to `/super-admin`.
- [ ] As `superadmin@randevulari.com`, try navigating to `/admin`. Verify restriction or behavior.
- [ ] As an unauthenticated user, verify `/admin` and `/super-admin` redirect to `/login`.

## 2. Onboarding Wizard Actions
- [ ] In `/admin`, open Kurulum SihirbazÄ±.
...
- [ ] Step 7: Verify "YayÄ±na HazÄ±r Olarak Ä°ÅŸaretle" is disabled if Step 1, 3, or 4 is incomplete.
- [ ] Step 7: Un-mock mode (simulate real data): Verify clicking "YayÄ±na HazÄ±r Olarak Ä°ÅŸaretle" sets status to `ready_for_review` and disables "YayÄ±na Al" for standard admin unless overridden by roles.
...
- [ ] Verify banner appears: "Hizmet GeÃ§ici Olarak KapalÄ±" (or similar) preventing bookings.
