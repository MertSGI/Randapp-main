...
export const tenantService = {
  ...
  async getCurrentTenant(): Promise<Tenant | null> {
    ...
    if (hash) {
      ...
      } else if (parts.length >= 3 && parts[1] === 'health-tourism') {
        urlSlug = parts[2].split('?')[0];
      } else if (parts.length >= 2 && parts[1] && parts[1] !== 'book' && parts[1] !== 'admin' && parts[1] !== 'super-admin' && parts[1] !== 'login' && parts[1] !== 'features' && parts[1] !== 'pricing' && parts[1] !== 'mobile-app' && parts[1] !== 'register' && parts[1] !== 'contact' && parts[1] !== 'pilot' && parts[1] !== 'privacy' && parts[1] !== 'terms' && parts[1] !== 'support' && parts[1] !== 'demo' && parts[1] !== 'customer' && parts[1] !== 'appointment' && parts[1] !== 'health-tourism') {
